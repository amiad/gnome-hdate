...
export class Hdate {
    ...
    hdate_get_year_type(size_of_year, new_year_dw) {
        ...
        return (offset > 0 && offset <= year_types.length) ? year_types[offset - 1] : 0;
    }
    ...
    hdate_jd_to_hdate(jd) {
        ...
        if (jd < jd_tishrey1) {
            year--;
            jd_tishrey1_next_year = jd_tishrey1;
            jd_tishrey1 = this.hdate_days_from_3744(year) + 1715119;
        } else if (jd >= jd_tishrey1_next_year) {
            year++;
        ...
        if (days >= (size_of_year - 236)) {
            let d2 = days - (size_of_year - 236);
            month = Math.floor(d2 * 2 / 59);
            day = d2 - Math.floor((month * 59 + 1) / 2) + 1;
            month += 5;
    ...
    set_jd(jd) {
        ...
        this.hd_weeks = Math.floor((this.hd_days + this.hd_new_year_dw - 2) / 7) + 1;
    ...
    numToHebrew(num, isYear = false) {
        ...
        for (let item of letters) {
            while (n >= item.v) { str += item.l; n -= item.v; }
        }
