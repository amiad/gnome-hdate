...
export function hdate_get_holyday(h, isDiaspora = false) {
    ...
    if (holyday === 26 && h.hd_gyear < 1968) holyday = 0; // Yom Yerushalayim

    if (holyday === 17) { // Independence Day & Memorial Day logic
        if (h.hd_gyear < 1948) holyday = 0;
        else if (h.hd_gyear < 2004) {
    ...
    if (holyday === 36 && (h.hd_gyear < 2005 || (h.hd_day === 30 && h.hd_dw !== 1) || (h.hd_day === 29 && h.hd_dw === 7))) holyday = 0;
